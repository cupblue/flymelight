...
　　}
	...
	function showRes(data){
		...
		data = {'status' : 2, 'total' : data.total, 'timesleft' : 0, 'result' : '', 'percent' : '52.7', 'creditsTotal' : 100, 'creditsConsume' : 20, 'shareDone' : 0, 'suc' : 1, 'przTit':'人品爆发~', 'przCont' : '攻城掠地礼包', 'tips' : '' };
		var pophtml = '<div class="popup-tit">游戏结束</div>' +
					  '<div class="popup-cont">你为母亲抢到了 <i>' + data.total + '</i> 朵花，<br>战胜了全宇宙 <i>' + data.percent + '%</i> 的小伙伴~<br>同时还获得了：<br><img src="images/gift01.png" alt=""><br><span class="gift-name">' + data.przCont +  '</span></div>' +
					  '<div class="popup-btn tc"><a href="javascript:;" class="cbtn-a cbtn-lft btn-showoff">炫耀一下</a><a href="javascript:;" class="cbtn-a cbtn-rgt btn-view">立即领取</a></div>';
		setTimeout(function(){
	...
	function countdown(container, seconds){
		...
		var itimer = setInterval(function(){
			...
			container.innerHTML = '<b class="min">00</b>' + secstr;
			if(seconds <= 0){
	...
	function generateRadom(collection){
		...
		if(len > 0){
			...
			var	rdmNum = Math.floor(Math.random()*len/2 + 2);	//随机出现个数
			for(var j=1; j<=len; j++){
				arr.push(j);
			}
